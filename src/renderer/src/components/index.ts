export { FileTree } from './FileTree'
export type { FileInfo } from './FileTree'
export { VirtualizedMarkdown } from './VirtualizedMarkdown'
export { TabBar } from './TabBar'
export type { Tab } from './TabBar'
export { LoadingPlaceholder } from './LoadingPlaceholder'
export { MissingFilePlaceholder } from './MissingFilePlaceholder'
export { WorkspaceSwitcher } from './WorkspaceSwitcher'
export { WorkspaceImportControl } from './WorkspaceImportControl'
export { SearchBar } from './SearchBar'
export type { SearchBarHandle } from './SearchBar'
export { ErrorBoundary } from './ErrorBoundary'
export { ToastContainer } from './Toast'
export type { ToastMessage, ToastType } from './Toast'
export { ThemeToggle } from './ThemeToggle'
export { FolderHistoryDropdown } from './FolderHistoryDropdown'
export { RecentFilesDropdown } from './RecentFilesDropdown'
export { SettingsPanel } from './SettingsPanel'
export { default as FloatingNav } from './FloatingNav'
export { default as BacklinksPanel } from './BacklinksPanel'
export type { BacklinkItem } from './BacklinksPanel'
export { default as ReadAloudBar } from './ReadAloudBar'
export { default as TocPanel } from './TocPanel'
export { BookmarkPanel } from './BookmarkPanel'
export type { Bookmark } from './BookmarkPanel'
export { BookmarkBar } from './BookmarkBar'
export { Header } from './Header'
export { NavigationBar } from './NavigationBar'

// v1.4.0: 页面内搜索
export { InPageSearchBox } from './search'
export type { InPageSearchBoxProps } from './search'

// v1.4.0: 快捷键帮助弹窗
export { ShortcutsHelpDialog } from './ShortcutsHelpDialog'

// v1.5.1: 图片 Lightbox 预览
export { ImageLightbox } from './ImageLightbox'
export type { LightboxState } from './ImageLightbox'

// v1.5.1: 递归分屏面板
export { SplitPanel } from './SplitPanel'
export type { SplitPanelProps } from './SplitPanel'

// v1.7.0: DOCX 导出进度面板
export { ExportTaskView } from './ExportTaskView'
export { PreflightPanel } from './PreflightPanel'
export { MoveToDialog } from './MoveToDialog'
export { RenameRepairDialog } from './RenameRepairDialog'
export { FileTreeMoveRepairDialog } from './FileTreeMoveRepairDialog'

// 轻量 Markdown 编辑
export { QuickEditDrawer } from './QuickEditDrawer'
export * from './editor'
