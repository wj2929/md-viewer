...
两种方式都不会覆盖目标已存在的同名文件（此时会提示失败、源文件保留），也不会移动到系统敏感目录（如 `.ssh`、`.aws` 等受保护路径会被拦截）。
...
> 说明：本版本暂不恢复文档滚动位置，也不持久化分屏布局（只记忆标签列表与当前文档）。多窗口同时打开同一文件夹时，标签会话按“最后切走的窗口优先”记忆。
...
大目录或大文件搜索时，MD Viewer 会限制单轮扫描规模。超过阈值时仍返回已扫描结果，但会显示“只显示部分结果”等降级提示，建议缩小目录或更换关键词。
...
| Structurizr / Plotly / DBML / AntV G6 | 支持 | 支持 | 支持 | 支持 | 支持 | 支持 | 支持 |
