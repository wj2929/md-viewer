...
**当前状态**：v2.3.0 已覆盖常见块编辑、未保存草稿保护、撤销重做、保存后滚动保持和分屏编辑。复杂块继续保留源码编辑兜底。
...
**当前状态**：v2.3.0 重点验证桌面端 HTML/PDF、图表 PNG 下载和批量打包导出；DOCX 的高质量一致性继续由 `md-viewer-docx-service` 版本共同决定。
