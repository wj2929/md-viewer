...
### 新增
- **Tab 右键菜单** - 关闭当前/其他/所有/左侧/右侧，在 Finder 中显示，复制路径
- **Markdown 右键菜单** - 导出 HTML/PDF，复制为 Markdown/纯文本/HTML
- **多文件选择** - Cmd+点击切换选择，Shift+点击范围选择，Cmd+A 全选
- **跨应用剪贴板** - 支持 Finder ↔ MD Viewer 文件复制粘贴（macOS/Windows/Linux）
- **可调侧边栏** - 拖拽分隔条调整宽度（180-500px）
- **GitHub 风格样式** - 移植 Markdown Preview Enhanced 的 GitHub 主题
- **CI/CD 自动发布** - GitHub Actions 三平台并行构建，Tag 推送自动创建 Release

### 改进
- **安全加固** - PROTECTED_PATTERNS 扩展到 45+ 条规则
- **剪贴板重构** - 单一数据源架构，事务性粘贴
- **文件监听优化** - 简化为当前目录监听，避免 EMFILE 错误

### 修复
- 禁用虚拟滚动（分段渲染存在 Bug）
- 修复 CI TypeScript 类型错误
- 修复 Linux 打包缺少 author email
