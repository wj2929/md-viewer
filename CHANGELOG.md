...
> **状态**: 🚧 **开发中** | **类型**: macOS 安装体验 + 编辑/标签稳定性修复

### 🔧 改进
- macOS DMG 安装镜像新增 `首次打开说明.txt`，提示未公证开源应用首次打开时的处理方式。
- macOS DMG 显式设置 `MD Viewer.app`、`Applications` 和首次打开说明文件的位置，降低用户安装时遗漏说明的概率。
...
### 🧪 测试
- 新增 macOS DMG 配置回归测试，确保后续打包不会误删首次打开说明文件。
- 新增分屏编辑回归测试，覆盖历史编辑会话不自动弹旧快速编辑抽屉、明确 placement 才显示旧抽屉。
- 新增固定标签跨文件夹切换回归测试。
