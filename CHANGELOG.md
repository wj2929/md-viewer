...
### ✨ 新功能
- 新增**文件树拖放移动**：直接把文件/文件夹拖到目标目录完成移动，替代「剪切→切目录→粘贴」三步；支持多选批量拖动、拖到自身/子目录的前端预防（后端 `fs:moveFile` 边界校验兜底）；文件树无空白区，故在拖动进行中于顶部显示「拖回根目录」投放条（投放条挂载推迟到 dragstart 的下一帧，避免同步布局重排中止原生拖动）。目标限当前窗口根内（跨根移动另议）。
