...
> **状态**: ⏳ **待发布** | **类型**: 语音朗读 + CLI 多文件合并 + 多窗口阅读体验

v2.7.0 新增长文语音朗读、工作区与多窗口拆分/合并能力，并补齐多文件合并导出和跨目录整理体验。主要功能和文档已经完成，正式发布前仍需通过最后一轮完整实机检查。
...
- 补充工作区状态与安全边界测试，覆盖当前空工作区保护、无内容工作区自动清理、未保存编辑归属，以及窗口与页面状态保持一致。
...
- 发布前还将完成全量自动化测试、代码检查、应用构建、命令行与 DOCX 导出回归，以及最后一轮可见窗口实机操作检查；正式结果以本版本最终验证记录为准。
