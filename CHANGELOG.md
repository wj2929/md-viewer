...
> **状态**: 🚧 **开发中** | **类型**: 导出失败诚实化 + 路径授权收口 + macOS 安装体验

v2.6.0 有两条主线：一是**导出失败「看得见、不谎报、不漏源码」**——HTML/PDF/DOCX/CLI 导出在图表渲染失败时统一给出诚实状态与中性占位，不再把失败图表的源码漏进交付文件；二是**路径授权收口**——把进程级全局 `allowedBasePath` 收敛为「按发起窗口根目录」的单一事实源，消除多窗口下一个窗口切换文件夹污染另一窗口访问权的隐患。
