...
> **状态**: 🚧 **开发中** | **类型**: 文件夹级 tab 会话记忆 + tab 懒加载 + 最近项新窗口打开

### ✨ 新功能
- 文件夹会记住已打开的标签与当前文档：切换到别的文件夹再切回来时，自动恢复该文件夹上次打开的完整标签列表和当时正在看的文档；重启应用后也会恢复最近打开的文件夹及其标签。
- 最近文件、最近文件夹的右键菜单新增“在新窗口中打开”。

### 🔧 现有功能改进
- 恢复大量标签时改为懒加载：切回文件夹或恢复会话只重建标签壳，仅在点击某个标签（或它进入分屏）时才读取该文档内容，减少内存占用与切换卡顿；正在加载的内容区显示加载占位而非空白。
- 切换文件夹前若当前文档有未保存修改，会先提示保存，避免恢复体验让人误以为草稿已被完整保留。

### 📌 本期已知边界（后续迭代）
- 暂不恢复文档滚动位置。
- 暂不持久化分屏布局（只记忆标签列表与当前文档）。
- 多窗口打开同一文件夹时，标签会话按“最后切走的窗口优先”覆盖记忆（全局单份，非按窗口隔离）。
